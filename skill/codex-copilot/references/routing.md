...
- **L0**: Clear, local, low-risk micro-change with an obvious verification command. Use GPT-6 Sol Medium in the existing parent session, no subagent, and no independent review. New behavior or an ordinary feature is at least L1 even when it touches one module.
- **L1**: Normal change within one module. Use GPT-6 Sol Medium; add `copilot_scout` only when location or existing coverage is unclear; finish with `copilot_reviewer`.
...
- **L3**: Authentication, payment, permissions, migration, concurrency, security-sensitive, or otherwise high-blast-radius work. Reserve GPT-6 Sol High for one key decision or final review when the quota band permits it; the premium profile may instead reserve GPT-6 Astra High.
...
|---|---:|---|
| Green / standard | 5-hour ≥50% and weekly ≥35% | GPT-6 Sol Medium parent. L0 has no subagent; L1/L2 may use one Luna scout and one Sol final reviewer. L3 reserves its final review. |
...
| Red | 10-29% | L0: GPT-6 Luna Low. L1: GPT-6 Sol Low/Medium without delegation. Pause L2/L3. |
...
| Unknown | unavailable | GPT-6 Sol Medium, one read-only subagent maximum, no Astra/Max/Ultra. |
...
allows at most one read-only scout/investigator plus `copilot_final_reviewer` (GPT-6 Sol High).
...
For Green/standard L3, use at most one read-only investigator before implementation and reserve the other agent slot for the final review. The premium profile uses the dedicated GPT-6 Astra final reviewer; balanced and conservative use GPT-6 Sol High. Keep exactly one writer. The definition of done must cover the relevant permission matrix, migration compatibility, failure rollback or retry behavior, and representative existing data.
